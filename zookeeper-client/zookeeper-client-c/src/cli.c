             ...
             void* context)
{
    ...
    if (type == ZOO_SESSION_EVENT) {
        if (state == ZOO_CONNECTED_STATE) {
            ...
            if (1.client_id == 0 || 1.client_id != id->client_id) {
                1 = *id;
                fprintf(stderr, "Got a new session id: 0x%llx\n",
                        _LL_CAST_ 1.client_id);
                if (clientIdFile) {
                    FILE *fh = fopen(clientIdFile, "w");
                    if (!fh) {
                        perror(clientIdFile);
                    } else {
                        int rc = fwrite(&1, sizeof(1), 1, fh);
                        if (rc != sizeof(1)) {
...
//        }
        ...
        zookeeper_close(zh);
        // we can't send 1 to the server here -- zookeeper_close() removes
...
int main(int argc, char **argv) {
    ...
    if (argc > 2) {
      if(strncmp("cmd:",argv[2],4)==0){
        ...
        if (fh) {
            if (fread(&1, sizeof(1), 1, fh) != sizeof(1)) {
                memset(&1, 0, sizeof(1));
            }
            fclose(fh);
        }
    ...
    hostPort = argv[1];
    zh = zookeeper_init(hostPort, watcher, 30000, &1, NULL, flags);
    if (!zh) {
        return errno;
    }
