...
public class FinalRequestProcessor implements RequestProcessor {
    ...
    public void processRequest(Request request) {
        ...
        synchronized (zks.outstandingChanges) {
            ...
            // request.hdr设置为写请求，这是唯一添加到未完成更改的请求。
        ...
        // ZOOKEEPER-558:
        //       在某些情况下，服务器无法正确关闭连接（例如，closeconn缓冲区未排队-ZOOKEEPER-558）。
        //       例如，当客户端关闭连接时，就会发生这种情况。但是，服务器仍应关闭会话。
        //       丢失cnxn后调用closeSession（）会导致客户端关闭会话响应被丢弃。
        if (request.type == OpCode.closeSession && connClosedByClient(request)) {
            // 我们需要检查是否可以关闭会话ID。
            // 有时，对应的ServerCnxnFactory可能为null，因为我们只是在播放领导者的差异。
        ...
        try {
            if (request.getHdr() != null && request.getHdr().getType() == OpCode.error) {
                /*
                 *禁用本地会话升级后，领导者将由于会话过期而拒绝临时节点的创建。
                 * 但是，如果这是发出请求的关注者，则具有正确的错误代码，因此我们应该使用该并向用户报告
                 */
            ...
            // 根据不同的请求进行处理
            // 如果请求包含事务，该处理器将会接受对ZooKeeper数据树的修改，否则将会返回请求读取的数据
            switch (request.type) {
            case OpCode.ping: {
        ...
        try {
            ...
            // FinalRequestProcessor是最后一个处理器，在最后释放资源
