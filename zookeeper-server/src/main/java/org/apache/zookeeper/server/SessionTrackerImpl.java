...
public class SessionTrackerImpl extends ZooKeeperCriticalThread implements
    ...
    public void run() {
        try {
            ...
            while (running) {
                ...
                // poll 将 当前过期时间分片 从”桶集“中移除，返回移除的桶
                for (SessionImpl s : sessionExpiryQueue.poll()) {
                    // 遍历桶中的所有Session会话对象

                    // 设置 Closing = true
                    setSessionClosing(s.sessionId);
                    // 关闭会话，这里的expirer是ZooKeeperServer，ZooKeeperServer在创建SessionTrackerImpl时将this传入进来
                    expirer.expire(s);
                }
            }
    ...
    synchronized public boolean touchSession(long sessionId, int timeout) {
        ...
        // 被标识为关闭，打印日志 TODO 因此，在run()方法中就会将会话关闭，此处将直接退出
        if (s.isClosing()) {
