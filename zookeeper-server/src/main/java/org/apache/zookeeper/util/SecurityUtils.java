...
public final class SecurityUtils {
    ...
    /**
     * 创建SaslServer的实例。如果有异常，它将返回null。
     *
     * @param subject subject
    ...
    public static SaslServer createSaslServer(final Subject subject,
        ...
        if (subject != null) {
            // 服务器正在使用经过JAAS认证的主题：从zk服务器的主题确定服务主体名称和主机名。
            if (subject.getPrincipals().size() > 0) {
                try {
                    ...
                    // 服务主体名称和主机名 e.g. servicePrincipalNameAndHostname := "zookeeper/myhost.foo.com@FOO.COM"
                    ...
                    // TODO: 应该取决于zoo.cfg指定的机制，但是如果subject为非null，则可以认为它是GSSAPI。
                    ...
                    if (usingNativeJgss) {
                        // http://docs.oracle.com/javase/6/docs/technotes/guides/security/jgss/jgss-features.html
                        // """
                        // 另外，在以特定主题执行操作时，例如Subject.doAs（...）或 Subject.doAsPrivileged（...），
                        // 应将使用的GSSCredential添加到Subject的专用凭据集中。否则，由于找不到凭证，因此GSS操作将失败。
                        // """
                        try {
                            GSSManager manager = GSSManager.getInstance();
                            Oid krb5Mechanism = new Oid("1.2.840.113554.1.2.2");
                ...
                // JAAS非GSSAPI认证：目前仅假设并支持DIGEST-MD5机制。
                // TODO: 在zoo.cfg中使用 'authMech='
                try {
                    //
