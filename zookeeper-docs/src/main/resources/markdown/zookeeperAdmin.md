...
4. Create a configuration file. This file can be called anything.
  ...
  *1*, one for each server, which resides in
  that server's data directory, as specified by the configuration file
  parameter **dataDir**.
  
5. The 1 file
  consists of a single line containing only the text of that machine's
  id. So *1* of server 1 would contain the text
...
* *server.x=[hostname]:nnnnn[:nnnnn], etc* :
    ...
    file *1* in the data directory. That file
...
required directories, and optionally setup the 1 file
...
of the 1 file) has been an issue for users in the past.
...
* *1* - contains a single integer in
...
places: the *1* file and the configuration file.
The *1* file identifies the server that
...
*1* file and then, using that id, reads from the
