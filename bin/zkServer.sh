...
status)
    ...
    then
       ...
       1=`cat "$dataDir/1"`
       if ! [[ "$1" =~ ^[0-9]+$ ]] ; then
         echo "clientPort not found and 1 could not be determined. Terminating."
         exit 1
       fi
       clientPortAndAddress=`$GREP "^[[:space:]]*server.$1=.*;.*" "$ZOOCFG" | sed -e 's/.*=//' | sed -e 's/.*;//'`
       if [ ! "$clientPortAndAddress" ] ; then
           ...
           clientPortAndAddress=`$GREP "^[[:space:]]*server.$1=.*;.*" "$dynamicConfigFile" | sed -e 's/.*=//' | sed -e 's/.*;//'`
       fi
       if [ ! "$clientPortAndAddress" ] ; then
