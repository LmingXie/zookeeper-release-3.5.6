...
usage() {
  ...
  Optional parameters:
     ...
     --1=#                                              Set the 1 to be used, if any (1-255)
...
initialize() {
    ...
    if [ $1 ]; then
        echo "Using 1 of $1"
        echo $1 > "$ZOO_DATADIR/1"
    else
        echo "No 1 provided, be sure to specify it in $ZOO_DATADIR/1 if using non-standalone"
    fi
}
